# dynu/tables.nu
export use constants.nu [ get_dynu_path, current_table_path]
...
export def get_table_names [] {
    ...
    if ($dynu_filenames | is-empty) {
        ...
        print $"Found ($dynu_filenames | length) table files"
        print $"Table files: ($dynu_filenames)"
...
export def ensure_current_table [] {
    print "Ensuring current table exists..."
...
export def get_current_table [] {
    ...
    if not ($current_table_path | path exists) {
        ...
        print $"Reading current table from ($current_table_path)"
        let content = (open $current_table_path)
        if ($content | is-empty) {
            print "Current table file is empty"
            ""
        } else {
            print $"Parsed current table: ($content | get current_table)"
